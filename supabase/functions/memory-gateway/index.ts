...
Deno.serve(async (req: Request) => {
  ...
  const { data: guardRows, error: guardError } = await db.rpc(
    ...
    { p_token: runtimeKey, p_required_scope: requiredScope, p_endpoint: "memory-gateway" },
  );
  ...
  if (!guard?.allowed) {
    if (guard?.reason === "rate_limited") {
      return response(429, { error: "rate_limited", retry_after_seconds: guard.retry_after_seconds ?? 60 });
    }
